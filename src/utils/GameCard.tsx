...
export default function GameCard({ game }: { game: Game }) {
  ...
  return (
    <Link href={`/${game.id}`} className="block cursor-pointer">
          ...
          <div className="group/inner">
            <div className="aspect-4/3 w-full overflow-hidden">
              <img
                src={game.thumbnail}
                alt={game.title}
                className="h-full w-full object-cover transition duration-500 group-hover/inner:scale-105"
              />
            ...
            <div className="px-5 pb-5 pt-2 text-left">
              <div className="flex items-center justify-between gap-3">
                <div className="">
                  ...
                  <span className="rounded-full text-xs font-semibold text-cyan-700 dark:bg-cyan-500/15 dark:text-cyan-300">
                    {game.rate} {game.starRate}
                  </span>
                  <p className=" text-xs text-slate-500 dark:text-slate-400 text-clip line-clamp-2 ">
                    {game.subTitle}
                  </p>
                </div>
              </div>
